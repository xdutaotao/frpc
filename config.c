...
void free_common_config()
{
    ...
    if (c_conf->server_addr)
        free(c_conf->server_addr);
    if (c_conf->http_proxy)
        free(c_conf->http_proxy);
    if (c_conf->log_file)
        free(c_conf->log_file);
    if (c_conf->log_way)
        free(c_conf->log_way);
    if (c_conf->log_level)
        free(c_conf->log_level);
    if (c_conf->auth_token)
        free(c_conf->auth_token);
    if (c_conf->privilege_token)
        free(c_conf->privilege_token);
...
void free_base_config(struct base_conf *bconf)
{
    if (bconf->name)
        free(bconf->name);
    if (bconf->auth_token)
        free(bconf->auth_token);
    if (bconf->privilege_token)
        free(bconf->privilege_token);
    if (bconf->host_header_rewrite)
        free(bconf->host_header_rewrite);
    if (bconf->subdomain)
        free(bconf->subdomain);
}
...
static void dump_common_conf()
{
    ...
    debug(LOG_DEBUG,
          "Section[common]: {server_addr:%s, server_port:%d, auth_token:%s, privilege_token:%s, "
          "interval:%d, timeout:%d}",
          c_conf->server_addr, c_conf->server_port, c_conf->auth_token, c_conf->privilege_token,
...
static void dump_proxy_service(const int index, struct proxy_service *ps)
{
    ...
    debug(LOG_DEBUG, "Proxy service %d: {name:%s, local_port:%d, type:%s}", index, ps->proxy_name,
          ps->local_port, ps->proxy_type);
}

// dump所有的proxy-service服务
...
static void new_ftp_data_proxy_service(struct proxy_service *ftp_ps)
{
    ...
    if (!ps) {
        ...
        if (!ps) {
            debug(LOG_ERR, "cannot create ftp data proxy service, it should not happenned!");
            exit(0);
        }
...
#define TO_BOOL(v) strcmp(value, "true") ? 0 : 1
    ...
    if (MATCH_NAME("type")) {
        ...
        assert(ps->proxy_type);
    } else if (MATCH_NAME("local_ip")) {   //本地ip
        ps->local_ip = strdup(value);
        assert(ps->local_ip);
    } else if (MATCH_NAME("local_port")) {   //本地port
        ps->local_port = atoi(value);
    } else if (MATCH_NAME("use_encryption")) {   //加密与否
        ps->use_encryption = is_true(value);
    } else if (MATCH_NAME("remote_port")) {   //远端port
        ps->remote_port = atoi(value);
    } else if (MATCH_NAME("remote_data_port")) {   //远端数据port
        ps->remote_data_port = atoi(value);
    } else if (MATCH_NAME("http_user")) {   // http-user
        ps->http_user = strdup(value);
        assert(ps->http_user);
    } else if (MATCH_NAME("http_pwd")) {   // http-password
        ps->http_pwd = strdup(value);
        assert(ps->http_pwd);
    } else if (MATCH_NAME("subdomain")) {   //子域名
        ps->subdomain = strdup(value);
        assert(ps->http_pwd);
    } else if (MATCH_NAME("custom_domains")) {   //自定义域名
    ...
    } else if (MATCH_NAME("use_encryption")) {   //这里好像写重了
        ps->use_encryption = TO_BOOL(value);
    } else if (MATCH_NAME("use_compression")) {   //压缩
...
char *get_ftp_data_proxy_name(const char *ftp_proxy_name)
{
    ...
    char *ftp_data_proxy_name =
        (char *) calloc(1, strlen(ftp_proxy_name) + strlen(ftp_tail_data_name) + 1);
    assert(ftp_data_proxy_name);

    snprintf(ftp_data_proxy_name, strlen(ftp_proxy_name) + strlen(ftp_tail_data_name) + 1, "%s%s",
             ftp_proxy_name, ftp_tail_data_name);
