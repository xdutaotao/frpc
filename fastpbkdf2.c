 ...
 */
#define DECL_PBKDF2(_name, _blocksz, _hashsz, _ctx, _init, _update, _xform, _final, _xcpy,         \
                    _xtract, _xxor)                                                                \
    typedef struct {                                                                               \
        _ctx inner;                                                                                \
        _ctx outer;                                                                                \
                                                                                                   ...
                                                                                                   \
    static inline void HMAC_INIT(_name)(HMAC_CTX(_name) * ctx, const uint8_t *key, size_t nkey)    \
    {                                                                                              \
        /* Prepare key: */                                                                         \
        uint8_t k[_blocksz];                                                                       \
                                                                                                   \
        ...
        _update(&ctx->outer, blk_outer, sizeof blk_outer);                                         \
                                                                                                   ...
                                                                                                   \
    static inline void HMAC_UPDATE(_name)(HMAC_CTX(_name) * ctx, const void *data, size_t ndata)   \
    {                                                                                              \
        _update(&ctx->inner, data, ndata);                                                         \
    }                                                                                              \
                                                                                                   \
    static inline void HMAC_FINAL(_name)(HMAC_CTX(_name) * ctx, uint8_t out[_hashsz])              \
    {                                                                                              \
        ...
        _final(out, &ctx->outer);                                                                  \
                                                                                                   ...
                                                                                                   \
    /* --- PBKDF2 --- */                                                                           \
    static inline void PBKDF2_F(_name)(const HMAC_CTX(_name) * startctx, uint32_t counter,         \
                                       const uint8_t *salt, size_t nsalt, uint32_t iterations,     \
                                       uint8_t *out)                                               \
    {                                                                                              \
        uint8_t countbuf[4];                                                                       \
        ...
        _xtract(&result, out);                                                                     \
                                                                                                   ...
                                                                                                   \
    static inline void PBKDF2(_name)(const uint8_t *pw, size_t npw, const uint8_t *salt,           \
                                     size_t nsalt, uint32_t iterations, uint8_t *out, size_t nout) \
    {                                                                                              \
        assert(iterations);                                                                        \
        assert(out &&nout);                                                                        \
                                                                                                   \
...
DECL_PBKDF2(sha1, SHA_CBLOCK, SHA_DIGEST_LENGTH, SHA_CTX, SHA1_Init, SHA1_Update, SHA1_Transform,
            SHA1_Final, sha1_cpy, sha1_extract, sha1_xor)
...
DECL_PBKDF2(sha256, SHA256_CBLOCK, SHA256_DIGEST_LENGTH, SHA256_CTX, SHA256_Init, SHA256_Update,
            SHA256_Transform, SHA256_Final, sha256_cpy, sha256_extract, sha256_xor)
...
DECL_PBKDF2(sha512, SHA512_CBLOCK, SHA512_DIGEST_LENGTH, SHA512_CTX, SHA512_Init, SHA512_Update,
            SHA512_Transform, SHA512_Final, sha512_cpy, sha512_extract, sha512_xor)
